...
self-contained (standard library only, Python 3.8+; `oneliner.py` needs 3.8+
for the walrus operator) and prints one prime per line to stdout.
...
## fast.py — Sieve of Eratosthenes with Rosser bound

Computes an upper bound for the 1,000,001st prime using Rosser's theorem
(`p_n < n(ln n + ln ln n)` for n ≥ 6), allocates one bytearray up to that
...
time using a lazily-extended list of base primes up to `sqrt(current
position)`, so memory stays effectively constant no matter how far it runs.
...
- **Space:** O(√n) for the base primes plus one fixed-size window — in
  practice a few kilobytes growing negligibly slowly.
